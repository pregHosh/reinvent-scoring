...
from .scscore_f.standalone_model_numpy import *

class SCScore(BasePhysChemComponent):
    def __init__(self, parameters: ComponentParameters):
        super().__init__(parameters)
        model = SCScorer()
        #TODOS check the path
        self.scsmodel = model.restore(os.path.join(project_root, "scscore_f", 'models', 'full_reaxys_model_1024bool', 'model.ckpt-10654.as_numpy.json.gz'))
